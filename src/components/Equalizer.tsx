      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex space-x-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-xl border text-[10px] font-bold tracking-widest uppercase transition-all flex items-center space-x-2 ${autoEq ? 'bg-accent/20 border-accent/50 text-accent shadow-[0_0_15px_rgba(234,179,8,0.3)]' : 'bg-transparent border-white/10 text-white/40 hover:text-white'}`}
          >
            <Activity size={12} className={autoEq ? 'animate-pulse' : ''} />
            <span>AI Auto EQ</span>
          </button>
          <button
            onClick={() => { if (!autoEq) setEqGains(bands.map(() => 0)); }}
            className={`p-2 rounded-full transition-colors ${autoEq ? 'bg-white/5 text-white/20 cursor-not-allowed' : 'bg-white/5 text-white/50 hover:text-white'}`}
            disabled={autoEq}
          >
            <RotateCcw size={16} />
          </button>
          ...
          <div key={freq} className="flex flex-col items-center space-y-4 px-0.5">
            ...
            <div className="relative h-40 w-5 flex justify-center">
              ...
              <input
                ...
                value={eqGains[i]}
                onChange={(e) => {
                  if (!autoEq) handleSliderChange(i, parseFloat(e.target.value));
                }}
                className={`eq-slider appearance-none bg-transparent z-10 ${autoEq ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}
                disabled={autoEq}
                style={{
              ...
              <div
                className={`absolute w-2.5 h-1 rounded-full pointer-events-none transition-all duration-300 ${autoEq ? 'bg-accent shadow-[0_0_8px_rgba(234,179,8,1)]' : 'bg-white'}`}
                style={{
                  bottom: `${((eqGains[i] + 12) / 24) * 100}%`,
                  boxShadow: autoEq ? `0 0 12px ${accentColor}` : `0 0 8px ${accentColor}`
                }}
              />
            </div>
