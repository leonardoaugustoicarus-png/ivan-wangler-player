...
export default function App() {
  ...
  const handleSelectTrack = (file: File | any, shouldPlay: boolean = true) => {
    ...
    const fetchAIEQProfile = async (title: string, artist: string) => {
      ...
      try {
        const prompt = `Return exactly a valid JSON array of 15 numbers (between -12 and 12, can be decimals) representing an ideal parametric EQ mastering profile for the genre of the song '${title}' by '${artist}'. The bands are [20, 40, 63, 100, 160, 250, 400, 630, 1000, 1600, 2500, 4000, 6300, 10000, 20000] Hz. Only return the raw array string like [-2, 0, 1...], nothing else.`;
  ...
  return (
    ...
    >
      ...
      <Visualizer3D analyser={analyser} isPlaying={isPlaying} accentColor={accentColor} />
